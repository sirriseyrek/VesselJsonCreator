<template>
  <main class="card form-card">
    <h2 class="form-title">{{ t('formTitle3') }}</h2>

    <div class="machinery-row" v-for="item in machineryList" :key="item.key">
...
// Döngü için makine listesi tanımı
const machineryList = [
  { key: 'mainEngine' },
  { key: 'generators' },
  { key: 'boiler' },
...
// Değer arttırma (Maksimum 9)
const increment = (key) => {
  if (props.vesselData[key] < 9) {
    props.vesselData[key]++
  }
}

// Değer azaltma (Minimum 0)
const decrement = (key) => {
  if (props.vesselData[key] > 0) {
    props.vesselData[key]--
  }
}

// Manuel klavye giriş kontrolü (Sadece 1 basamaklı sayı ve 0-9 arası zorlaması)
...
/* Ok butonlarını (spinners) gizleme */
